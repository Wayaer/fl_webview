...
class WebViewTools: NSObject {
    ...
    public func openWebview(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        let urlData = args["urlData"] as? [String: Any?]
        if urlData != nil {
            let headers = urlData!["headers"] as! [String: String]?
            guard let url = URL(string: urlData!["url"] as! String) else {
                result(FlutterError(
        ...
        if !parentCtrl.presentedViewControllers!.contains(webView) {
            if presentationStyle == .modal {
                parentCtrl.presentAsModalWindow(webView)
            } else {
                parentCtrl.presentAsSheet(webView)
            }
